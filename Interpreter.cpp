...
void Interpreter::init(const ExpsT& query) {
    ...
    mOrigVars = Interpreter::getVariableNames(query);
}
...
std::experimental::optional<Substitution> Interpreter::nextAnswer() {
    while (!mSearchStack.empty()) {
        ...
        for (int i = mProgram->getRules().size() - 1; i >= 0; i--) {
            ...
            std::shared_ptr<Function> positiveLiteral = definiteClause->getLhs();
            ...
            if (mgu) {
                // resolve the clauses
                ExpsT nextQuery = Interpreter::unify(config.query, negativeLiteral, renaming.applyTo(definiteClause->getRhs()), *mgu);
...
bool Interpreter::contains(const ExpsT& exps, std::shared_ptr<Expression> exp) {
...
ExpsT Interpreter::unify(const ExpsT& query, std::shared_ptr<Expression> resolvedLiteral, const ExpsT& definiteClause, const Substitution& mgu) {
    ExpsT result;

    for (int i = 0; i < (int) definiteClause.size(); i++) {
        std::shared_ptr<Expression> exp = mgu.applyTo(definiteClause[i]);
        if (!Interpreter::contains(result, exp)) {
...
std::set<std::string> Interpreter::getVariableNames(const ExpsT& exps) {
